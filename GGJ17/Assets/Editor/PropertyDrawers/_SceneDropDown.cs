...
public class _SceneDropDownAttribute : PropertyDrawer {
	...
	string DrawScenePopup(Rect position, GUIContent label, string currentScene) {
		...
		GUIContent[] sceneNames = new GUIContent[scenes.Length];
		int currentIndex = 0;

		// Find the current
		for (int i = 0; i < scenes.Length; i++) {
			sceneNames[i] = new GUIContent((i > 0 ? "[" + (i - 1) + "]\t" : "") + scenes[i]);
		...
		// Draw
		int index = EditorGUI.Popup(position, label, currentIndex, sceneNames);
